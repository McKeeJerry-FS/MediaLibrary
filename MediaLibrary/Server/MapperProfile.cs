...
namespace MediaLibrary.Server
{
    public class MapperProfile : Profile
    {
        ...
        {
            // TODO: Person Mapping
            CreateMap<Data.Person, Shared.Models.PersonModel>()
                .ForMember(x => x.MoviesIds, map => map.MapFrom(
                    actor => actor.Movies.Select(x => x.MovieId)));
            CreateMap<Shared.Models.PersonModel, Data.Person>()
                ...
                .ForMember(x => x.Movies, map => map.MapFrom((model, entity) =>
                {
                    ...
                    movies.AddRange(model.MoviesIds.Where(x => !current.Contains(x))
                        .Select(x => new Data.MovieActor
                        {
                            MovieId = x
                        }));
            ...
            // TODO: Movie Mapping
            CreateMap<Data.Movie, Shared.Models.MovieModel>()
           ...
           .ForMember(x => x.ActorIds, map => map.MapFrom(movie => movie.Actors.Select(x => x.PersonId)));
            ...
            CreateMap<Shared.Models.MovieModel, Data.Movie>()
                .ForMember(x => x.Id, map => map.Ignore())
