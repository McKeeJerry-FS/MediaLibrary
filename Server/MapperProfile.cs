    ...
    public class MapperProfile : Profile
    {
        public MapperProfile() 
        {
            ...
            CreateMap<Data.Person, Shared.Model.PersonModel>()
                .ForMember(x => x.MovieIds, map => map.MapFrom(actor => actor.Movies.Select(x => x.MovieId)));
            ...
            CreateMap<Shared.Model.PersonModel, Data.Person>()
                ...
                .ForMember(x => x.Movies, map => map.MapFrom((model, entity) =>
                {
                    ...
                    movies.AddRange(model.MovieIds.Where(x => !current.Contains(x)).Select(x => new Data.MovieActor
                    {
                        MovieId = x,
                        
                    }));

                    movies.RemoveAll(x => !model.MovieIds.Contains(x.MovieId));
                    return movies;
                }));
